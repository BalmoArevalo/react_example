...
const Example3 = () => {
    ...
    const handleGuardarOnClick = () => {
        ...
        // del array del estado (extraído usando operador spread) más la nueva card
        ...
        <div>
            <div className='mb-4'>
                ...
                <div className='flex flex-col gap-1'>
                    ...
                    <textarea id="content" cols={30} rows={5} value={content}
                        onChange={handleContentOnChange} />
                </div>
                <button className='my-1'
                    onClick={handleGuardarOnClick}>Agregar</button>
            </div>
            <div className='grid grid-cols-4 gap-2'>
                {cards.map((card, index, arrayOfCards) => {
                    const handleEliminarOnClick = () => {
                        ...
                        const newCards = cards.filter(cardToDelete =>
                            cardToDelete.id !== idCardActual);
                        setCards(newCards)
                    }
                    return (<div key={card.id} className='flex flex-col gap-1'>
                        <Card title={card.title} content={card.content} />
                        {arrayOfCards.length !== 1 && <button
                            className='self-center'
                            onClick={handleEliminarOnClick}>Eliminar</button>}
                    </div>)
                })}
            </div>
        </div>
